...
if [ "$serverchan_notify_1" = "1" ] ; then
    ...
    if [ "$lastIP" != "$hostIP" ] && [ ! -z "$hostIP" ] ; then
        ...
        curl -s "http://sc.ftqq.com/$serverchan_sckey.send?text=PublicIPChanged@Newifi" -d "&desp=New Public IP: ${hostIP}" &
...
if [ "$serverchan_notify_2" = "1" ] ; then
    ...
    if [ -s "/tmp/var/newhostname不重复.txt" ] ; then
        ...
        curl -s "http://sc.ftqq.com/$serverchan_sckey.send?text=NewDevice@Newifi" -d "&desp=${content}" &
...
if [ "$serverchan_notify_4" = "1" ] ; then
    ...
    if [ -s "/tmp/var/newhostname不重复_上线.txt" ] ; then
        ...
        curl -s "http://sc.ftqq.com/$serverchan_sckey.send?text=DeviceOnline@Newifi" -d "&desp=${content}" &
    ...
    if [ -s "/tmp/var/newhostname不重复_下线.txt" ] ; then
        ...
        curl -s "http://sc.ftqq.com/$serverchan_sckey.send?text=DeviceOffline@Newifi" -d "&desp=${content}" &
...
if [ "$serverchan_notify_3" = "1" ] && [ "$resub" = "1" ] ; then
    ...
    if [ $(cat /tmp/var/osub) != $(cat /tmp/var/nsub) ] && [ -f /tmp/var/nsub ] ; then
        ...
        content="New Firmware: `cat /tmp/var/nsub | grep -v "^$"` %0A%0A Current Version: `cat /tmp/var/osub | grep -v "^$"` "
        logger -t "【微信推送】" "固件 新的更新：${content}"
        curl -s "http://sc.ftqq.com/$serverchan_sckey.send?text=NewFirmware@Newifi" -d "&desp=${content}" &
